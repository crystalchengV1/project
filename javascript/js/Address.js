function Dsy() {//  新建dsy对象
    this.Items = {};
}
Dsy.prototype.add = function (id, iArray) {// 新建dsy对象方法add
    this.Items[id] = iArray;
}
Dsy.prototype.Exists = function (id) { // 新建dsy对象方法Exists
    if (typeof(this.Items[id]) == "undefined") return false;
    return true;
}
var select = ["s1", "s2", "s3"];
var opt0 = ["省份", "地级市", "市、县级市、县"];//下拉框第一个
function setup() {
    for (i = 0; i < select.length - 1; i++){
        document.getElementById(select[i]).onchange =new Function("change("+(i+1)+");");
    }
        change(0);//只调用一次,将省份信息加载到下拉框
}
function preselect(pre) {//默认初始的省份（提供初始的省份名称，计算index值，并将当前省份selectedIndex=index+1）
    var index;
    ...
    if(index<provinces.length){
        document.getElementById("s1").selectedIndex=index+1;
        change(1);
    }
}
function change(v) {//v=0/1/2
    console.log("v=="+v);
    var str="0";
    for(i=0;i<v;i++){//V=1/2时执行
        str+=("_"+(document.getElementById(select[i]).selectedIndex-1));
    }
    var s=document.getElementById(select[v]);//获取下拉框对象
    console.log(s);
    with(s){
        length=0;
        options[0]=new Option(opt0[v],opt0[v]);//下拉选项的文本值和value(获取下拉选框第一个值)
        if(v &&document.getElementById(select[v-1]).selectedIndex>0 ||!v){
            if(dsy.Exists(str)){
                ar=dsy.Items[str];
                for(var i=0;i<ar.length;i++){
                    options[length]=new Option(ar[i],ar[i]);
                    if (v)options[0].selected = false;
                }
            }
        }
        if (++v<select.length){change(v);}
    }
